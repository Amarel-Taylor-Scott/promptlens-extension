...
document.addEventListener('DOMContentLoaded', async () => {
  const [settings, { history = [] }, { cachedLimits }] = await Promise.all([
    chrome.storage.sync.get(['authMode', 'email', 'apiKey']),
    chrome.storage.local.get('history'),
    chrome.storage.local.get('cachedLimits'),
  ]);
  ...
  // Refresh limits from API in the background
  ...
  document.getElementById('open-site').addEventListener('click', (e) => {
    e.preventDefault();
    chrome.tabs.create({ url: PROMPTLENS_URL });
  });
