...
async function generatePrompt(imageUrl, tab) {
  ...
  try {
    if (/^(moz-extension|about|chrome):/.test(imageUrl)) {
...
async function saveHistory(entry) {
  ...
  await browser.storage.local.set({ history: history.slice(0, MAX_HISTORY) });
}
...
/* ------------------------------------------------------------------ */
/*  Toast via content script  (+ notification fallback)                */
/* ------------------------------------------------------------------ */
...
async function notifyTab(tabId, data) {
  try {
    ...
    await browser.tabs.sendMessage(tabId, { type: 'promptlens-toast', ...data });
  } catch {
    ...
    if (data.status === 'success' && data.url) {
      ...
      await browser.storage.local.set({ [`notif_${notifId}`]: data.url });
...
browser.notifications.onClicked.addListener(async (notifId) => {
  ...
  if (stored[key]) {
    browser.tabs.create({ url: stored[key] });
